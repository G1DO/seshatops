...
| `CLM-003` | Transactional outbox handling preserves accepted source intent during broker outage. | Observed | `CAP-005` | M1 | Outbox integration test and outage fault campaign | Test report and raw delivery trace | [M1_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/M1_EXIT_GATE_EXPERIMENT_REPORT.md); FC-007 | Test environment | `a4e5d47` (runtime) + Issue #30 PR head | 2026-08-12 | Implementation campaign pass; maintainer review follow-up | Single-host Testcontainers Redpanda stop/start only; not staging/production outage or capacity evidence. | — | — | Issue #30 EXP-M1-EXIT-GATE-001. |
| `CLM-004` | Event processing is duplicate-safe. | Observed | `CAP-007` | M1 | Duplicate-delivery integration test and fault report | Test report and deduplication trace | [M1_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/M1_EXIT_GATE_EXPERIMENT_REPORT.md); FC-001/012/013 | Test environment | `a4e5d47` (runtime) + Issue #30 PR head | 2026-08-12 | Implementation campaign pass; maintainer review follow-up | One durable inventory-projection effect under tested duplicates/crash windows; **not** exactly-once delivery or multi-node evidence. | — | — | Must remain distinct from exactly-once delivery claims. |
| `CLM-005` | Versioned event contracts and ordering compatibility are enforced. | Observed | `CAP-006` | M1 | Contract and compatibility tests | Compatibility report and event trace | [M1_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/M1_EXIT_GATE_EXPERIMENT_REPORT.md); FC-009/010/011 | Test environment | `a4e5d47` (runtime) + Issue #30 PR head | 2026-08-12 | Implementation campaign pass; maintainer review follow-up | Exact schema v1 + M1 gap/stale/reorder/poison dispositions only; not a general migration or operator quarantine product. | — | — | Unsupported versions and ordering gaps require explicit dispositions. |
| `CLM-006` | Unchanged history deterministically rebuilds projections. | Observed | `CAP-007` | M1 | Deterministic projection rebuild test and expected-state comparison | Projection rebuild report and expected-state artifact | [M1_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/M1_EXIT_GATE_EXPERIMENT_REPORT.md); FC-014 | Test environment | `a4e5d47` (runtime) + Issue #30 PR head | 2026-08-12 | Implementation campaign pass; maintainer review follow-up | Platform `ResetDerivedState`/`RebuildFromHistory` A==B for complete retained history; not M3 backup/restore (ADR-Q-005). | — | — | M3 requires a separate claim for authorized recovery replay/restore. |
