...
| `CLM-005` | Versioned event contracts and ordering compatibility are enforced. | Planned | `CAP-006` | M1 | Contract and compatibility tests | Compatibility report and event trace | — | Planned | — | — | — | Compatibility policy exists in CONTRACTS.md; Issue #22 adds local contract tests under `event/`; Issue #24 publishes exact stored bytes to Redpanda with the M1 topic/key policy, but consumer ordering/inbox enforcement is not implemented. | — | — | Unsupported versions and ordering gaps require explicit dispositions. Do not promote from unit tests alone. |
