...
| `CLM-003` | Transactional outbox handling preserves accepted source intent during broker outage. | Planned | `CAP-005` | M1 | Outbox integration test and outage fault campaign | Test report and raw delivery trace | — | Planned | — | — | — | Issue #24 adds relay library/integration tests including broker-outage persistence and recovery; claim is not promoted. | — | — | Targets and outage conditions remain to be declared later. |
| `CLM-004` | Event processing is duplicate-safe. | Planned | `CAP-007` | M1 | Duplicate-delivery integration test and fault report | Test report and deduplication trace | — | Planned | — | — | — | Issue #25/#29 add `platform` consumer tests for identical redelivery, post-commit/pre-ack crash windows, and FC-001-style duplicate injection with unchanged checksum; claim is not promoted to Observed/Reproduced. | — | — | Must remain distinct from exactly-once delivery claims. |
| `CLM-005` | Versioned event contracts and ordering compatibility are enforced. | Planned | `CAP-006` | M1 | Contract and compatibility tests | Compatibility report and event trace | — | Planned | — | — | — | Compatibility policy exists in CONTRACTS.md; Issue #22/#24 cover library and publication; Issue #25/#26 enforce aggregate-version gap/stale/reorder rules, unsupported-schema quarantine, and sanitized failure visibility in `platform` tests. Claim is not promoted. | — | — | Unsupported versions and ordering gaps require explicit dispositions. Do not promote from unit tests alone. |
| `CLM-006` | Unchanged history deterministically rebuilds projections. | Planned | `CAP-007` | M1 | Deterministic projection rebuild test and expected-state comparison | Projection rebuild report and expected-state artifact | — | Planned | — | — | — | Issue #25 implements `platform.inventory_projection` and `ChecksumTenant`; Issue #29 adds `ResetDerivedState`, `RebuildFromHistory`, and local FC-014-style `A == B` proof with incomplete gating for unsafe history. Full hosted campaign evidence remains later. | — | — | M1 owns the initial projection rebuild claim; M3 will require a separate future claim for authorized recovery replay and checksum reconstruction. |
