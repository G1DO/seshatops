...
| `CLM-005` | Versioned event contracts and ordering compatibility are enforced. | Planned | `CAP-006` | M1 | Contract and compatibility tests | Compatibility report and event trace | — | Planned | — | — | — | Compatibility policy exists in CONTRACTS.md; Issue #22/#24 cover library and publication; Issue #25 enforces aggregate-version gap/stale rules and unsupported-schema quarantine in `platform` tests. Claim is not promoted. | — | — | Unsupported versions and ordering gaps require explicit dispositions. Do not promote from unit tests alone. |
