...
| `CLM-007` | Tenant isolation holds on Identity & Operations HTTP surfaces (inventory, ops, quarantine, replay, rebuild, audit). | Observed | `CAP-010` | Identity | Cross-tenant negative suite | Security test report and denial result table | [IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md) | Test environment | `173cedb` (runtime); docs on Issue #50 PR head | 2026-08-13 | G1DO (exit-gate suites; same person as operator) | Inventory, ops, quarantine, replay, rebuild, and audit HTTP only. Not retrieval, citations, approvals, commands, exports, caches, indexes, or backup/restore. Not production isolation. | — | — | Constitution-era note that scope must include retrieval/citations/approvals/commands/exports remains for later sequences; those categories are Not applicable in EXP-M2-EXIT-GATE-001. Hosted CI not yet recorded. |
| `CLM-008` | Privileged Identity HTTP operations are default-deny. | Observed | `CAP-010` | Identity | Privileged-operation negative suite | Security test report and audit evidence | [IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md) | Test environment | `173cedb` (runtime); docs on Issue #50 PR head | 2026-08-13 | G1DO (exit-gate suites; same person as operator) | Default-deny on `MX-004`–`MX-007` plus forged/stale session and revoked assignment in library/test scope. Not production IdP/revocation or a policy-engine product. | — | — | Named frozen suite includes missing/ambiguous membership, nil policy, unassigned principals, and SSE stop after assignment revoke. Hosted CI not yet recorded. |
| `CLM-009` | Authorized tenant-scoped ops visibility is available. | Observed | `CAP-012` | Identity | Operational visibility integration test | Security/operational test report | [IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md) | Test environment | `173cedb` (runtime); docs on Issue #50 PR head | 2026-08-13 | G1DO (exit-gate suites; same person as operator) | Authorized tenant-scoped `GET /v1/tenants/{tenant_id}/ops` (`MX-002`/`MX-003`) in library/test scope. Not an SLO/alerting platform or telemetry trace bundle. | — | — | Visibility does not itself establish availability or SLO compliance. Hosted CI not yet recorded. |
| `CLM-010` | Quarantine release, replay, and rebuild are authorized and fail closed on Identity HTTP. | Observed | `CAP-013` | Identity | Privileged-operation authorization tests | Security test report, quarantine record, and audit trace | [IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md](docs/evaluation/IDENTITY_OPERATIONS_EXIT_GATE_EXPERIMENT_REPORT.md) | Test environment | `173cedb` (runtime); docs on Issue #50 PR head | 2026-08-13 | G1DO (exit-gate suites; same person as operator) | Authorized same-tenant quarantine-release/replay/rebuild POSTs; gap not releasable; audit insert failure blocks mutation; no global `ResetDerivedState`. Not Traceability restore; not irreversible-external-effect safety. `FC-015`/`FC-016` remain Planned. | — | — | Authorization subset only; reliability fault campaign is not this issue. Hosted CI not yet recorded. |
