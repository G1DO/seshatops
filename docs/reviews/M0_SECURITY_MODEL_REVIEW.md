...
| Date (UTC) | 2026-08-06 |
| Branch / base | docs/5-threat-authorization-model; base commit 6a0fbf9; working-tree changes reviewed before commit |
...
| Boundaries | Browser-Go, Go-PostgreSQL, Go-Redpanda, Go-object storage, Go-Python, Go-adapter, Python-approved inputs, asynchronous publisher/consumer, tenant, administrative/operational |
...
| T-03 Confused deputy | A-01, A-03, A-04, A-05, A-06 | AUTH-07, AUTH-12 | Use an authorized service with an unauthorized initiating principal, tenant, target, action, or scope | Issue #5; later service/adapters |
| T-04 Stale authority | A-02, A-03, A-04, A-05 | AUTH-05, AUTH-06, AUTH-07, AUTH-11 | Change role, scope, membership, session, policy, or target state between checkpoints | Issue #5; later identity/operations |
| T-05 Replayed commands/approvals | A-04, A-05, A-10 | AUTH-05, AUTH-06; CM-02 to CM-04 | Replay after retry, timeout, restart, and redelivery; verify one durable effect | Issue #4, Issue #5, adapters, Issue #7 |
| T-06 Approval substitution/parameter change | A-03, A-04, A-05 | AUTH-05, AUTH-06, AUTH-11 | Alter tenant, target, action, material parameter, intent, or target version after approval | Issue #5; later workflow |
| T-07 Compromised session/token | A-02, A-03, A-04, A-05, A-09 | AUTH-01, AUTH-05, AUTH-06 | Exercise expired, revoked, altered, wrong-tenant, and stolen-session fixtures | Later identity/operations; Issue #7 |
| T-08 Service over-privilege/compromise | A-01, A-03, A-05, A-06, A-08 | AUTH-07, AUTH-12 | Attempt each service identity's disallowed tenant, read, write, approve, command, storage, and audit operation | Issue #5; later deployment |
...
| T-17 Payload/integrity conflict | A-01, A-05, A-06, A-07 | AUTH-05, AUTH-06, AUTH-10; EM-06; CM-03 | Reuse event, command, approval, receipt, proposal, or idempotency identity with changed content | Issue #4, Issue #5; later implementation |
...
| --- | --- | --- |
| AUTH-01 Missing, unknown, invalid, stale, or ambiguous context defaults to deny | AUTHORIZATION_MODEL.md sections 3 and 5 | T-01, T-04, T-07, T-19 |
| AUTH-02 No tenant can read or affect another tenant | AUTHORIZATION_MODEL.md section 6 | T-01, T-02, T-11, T-16, T-18, T-19 |
| AUTH-03 Go owns authoritative authorization | AUTHORIZATION_MODEL.md sections 3 and 5 | T-02, T-03, T-09, T-16 |
| AUTH-04 Browser visibility never grants authority | AUTHORIZATION_MODEL.md sections 2 and 5 | T-02, T-07, T-13 |
| AUTH-05 Authentication does not imply authorization | AUTHORIZATION_MODEL.md sections 2 and 3 | T-04, T-07 |
| AUTH-06 Approval does not replace execution-time authorization | AUTHORIZATION_MODEL.md sections 2 and 7 | T-05, T-06, T-15 |
| AUTH-07 Checks occur at display, approval, and execution | AUTHORIZATION_MODEL.md section 7 | T-04, T-05, T-06, T-07, T-18 |
...
| AUTH-12 Service identity preserves initiating context | AUTHORIZATION_MODEL.md sections 3, 8, and 9 | T-03, T-08, T-16 |
...
4. Role, scope, policy, session, approval, and target-version changes between display, approval, and execution.
5. Replayed and concurrent commands or approvals.
6. Changed material parameters under a reused approval or idempotency key.
7. Over-privileged or compromised service identities.
8. Python attempts to write state, authorize, command, or cross tenants.
9. Prompt injection and malicious retrieved instructions.
10. Unauthorized, stale, conflicting, and misleading citations.
11. Event and command tenant-context mismatch.
12. Tampered payloads and conflicting logical identities.
13. Forged, altered, uncertain, and wrong-tenant receipts.
14. Audit deletion, alteration, omission, and forgery.
15. Sensitive exposure through errors, logs, traces, exports, quarantine, and evidence.
16. Authorization work amplification and availability abuse.
...
| Threat-row field completeness | Pass; 19 threat entries each contain all required fields |
