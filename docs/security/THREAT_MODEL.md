...
2. **Go to PostgreSQL:** authoritative state and governance reads/writes cross into the transactional store. A database capability must not be treated as permission to bypass Go policy.
3. **Go to Redpanda:** event publication or consumption crosses an asynchronous transport boundary. Event tenant context, producer identity, payload integrity, and version meaning require validation.
4. **Go to object storage:** evidence, exports, documents, and other artifacts cross a storage boundary. Object identity and storage access do not grant tenant or business authority.
5. **Go to Python:** approved feature or evidence inputs cross into advisory intelligence. Python output returns as data, not as executable authority.
6. **Go to external operational adapters:** a command crosses into an independent operational system. Go must bind tenant, principal, action, target, intent, approval, and idempotency before sending it.
7. **Python to approved read-only or evidence inputs:** retrieval and feature inputs are authorized before content reaches Python. Retrieved content is untrusted data, not instructions.
8. **Asynchronous publisher and consumer boundaries:** retries, duplicate deliveries, stale messages, conflicting identities, and tenant mismatches must not silently become accepted business effects.
9. **Tenant boundary:** every read, write, retrieval, citation, approval, command, export, audit lookup, replay, and recovery action must remain within its authorized tenant.
10. **Administrative and operational boundary:** platform-level recovery, audit, and health duties must not silently become unrestricted tenant business access.
...
- **Preventive controls:** AUTH-07 least-privilege service identities; AUTH-12 validate caller service, initiating principal, tenant, resource, action, scope, approval, command binding, and lineage.
...
- **Preventive controls:** AUTH-05 current authorization and freshness at approval; AUTH-06 execution-time recheck; policy/assignment version and session freshness are current inputs.
...
- **Preventive controls:** AUTH-05 bound approval freshness; AUTH-06 execution-time authorization; Issue #4 CM-02 through CM-04 business-intent idempotency.
...
- **Preventive controls:** AUTH-05 bind approval to tenant, action, target, important parameters, normalized intent, target version, approver, expiry, and revocation state.
...
- **Preventive controls:** Future authentication integration with bounded sessions; AUTH-01 default deny; current authorization, tenant binding, freshness, and approval/execution checks.
...
- **Preventive controls:** AUTH-07 explicit least privilege; separate service responsibilities; service credentials do not substitute for user authority; audit writers cannot gain business-state authority.
...
- **Preventive controls:** Validate tenant against aggregate, target, authorization context, command intent, and processing context; quarantine invalid or missing context.
...
- **Preventive controls:** Validate identity/content equivalence; reject conflicting event IDs or idempotency intent; bind approvals and receipts to material intent and target version.
...
- **Preventive controls:** AUTH-02 tenant isolation; least-privilege display/export/audit access; minimize and classify logged data; never treat traces or evidence as unrestricted.
...
- **Preventive controls:** Fail closed on missing authorization; bounded work and backpressure; isolate tenant and operational work; do not silently accept work that cannot be safely retained.
...
| --- | --- | --- |
| T-01, T-02, T-11, T-16 | AUTH-01, AUTH-02, AUTH-03, AUTH-09 | Issue #5, Issue #6, later identity/retrieval implementation |
| T-03, T-08 | AUTH-07, AUTH-12 | Issue #5, later service/adapter implementation |
| T-04, T-05, T-06, T-07 | AUTH-05, AUTH-06, AUTH-11; CM-02 through CM-06 | Issue #4, Issue #5, later identity/workflow implementation |
| T-09, T-10, T-13 | AUTH-08, AUTH-09 | Issue #5, Issue #6, later intelligence implementation |
| T-12 | AUTH-09 | Issue #6 |
| T-14, T-15, T-17 | AUTH-10; EM-06; CM-03, CM-07, CM-08 | Issue #4, Issue #5, Issue #7 |
