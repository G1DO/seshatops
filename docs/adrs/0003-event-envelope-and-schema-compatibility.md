...
1. Event Spine uses strict UTF-8 JSON and the repository-owned contract in CONTRACTS.md. Duplicate object member names, non-JCS-compatible values, and non-integer or out-of-range v1 numeric forms are rejected before canonicalization.
...
Rejected for Event Spine because code generation and compatibility tooling add immediate
scope without a demonstrated requirement.

### A schema-registry service

Rejected because Event Spine has one event family and one supported version; a registry
