...
* Relay degradation: `seshatops_relay_publish_outcomes_total{outcome="transient"}` increments, structured logs `relay.cycle.failed` with `worker="relay"` and bounded `duration_ms`. Consumer idle: `consumer.cycle.completed` stops emitting `processed`, `seshatops_consumer_observed_lag_known 0`.
